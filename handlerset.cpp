...
CHandlerSet::CHandlerSet()
{
	...
	int ret;
	ret = m_tempinfo.ResetParams();
	_ASSERT( !ret );
	cur_seri = 0;
	m_depth = 0;
	m_shdtype = 0;
	ZeroMemory( m_tempname, 256 );
...
int CHandlerSet::LoadSigFile( char* fname, int adjustuvflag, float srcmult )
{
	...
	D3DXVECTOR3 multvec = D3DXVECTOR3( srcmult, srcmult, srcmult );
	D3DXVECTOR3 offsetpos( 0.0f, 0.0f, 0.0f );
	D3DXVECTOR3 rot( 0.0f, 0.0f, 0.0f );
...
//time3 = timeGetTime();
	/***
	ret = m_shandler->CreateBSphereData( m_mhandler->m_kindnum, *framemaxptr + 1 );
	if( ret ){
		DbgOut( "HandlerSet : LoadQuaFileFromBuf : CreateBSphere error !!!\n" );
		_ASSERT( 0 );
		return 1;
	}
...
//time4 = timeGetTime();
	...
	ret = m_shandler->SetBSphereData( m_mhandler, *cookieptr, 0, *framemaxptr );
	if( ret ){
		DbgOut( "HandlerSet : LoadQuaFileFromBuf : SetBSphere error !!!\n" );
		_ASSERT( 0 );
		return 1;
	}
	***/
//time5 = timeGetTime();
	...
	/***
	DWORD diff1, diff2, diff3, diff4, diff5;
	diff1 = time1 - time0;
	diff2 = time2 - time1;
	diff3 = time3 - time2;
	diff4 = time4 - time3;
	diff5 = time5 - time4;
	***/
//DbgOut( "HandlerSet : LoadQuaFile : CheckLoadTime : diff1 %d, diff2 %d, diff3 %d, diff4 %d, diff5 %d\n",
...
/***
int CHandlerSet::LoadMotFile( char* fname, int* cookieptr, int* framemaxptr )
{
	CMotFile* motfile;
	motfile = new CMotFile();
	if( !motfile ){
		_ASSERT( 0 );
		return 1;
	}
	...
	char motname[256];
	ZeroMemory( motname, 256 );
	int framenum, mottype;
	*cookieptr = motfile->LoadMotData( m_mhandler, fname, motname, &framenum, &mottype );
	if( *cookieptr < 0 ){
		DbgOut( "HandlerSet : LoadMotFile : LoadMotData error !!!\n" );
		return 1;
	}
	if( framenum > 0 )
		*framemaxptr = framenum - 1;
	else
		*framemaxptr = 0;
...
int CHandlerSet::LoadMotFileFromBuf( char* bufptr, int bufsize, int* cookieptr, int* framemaxptr )
{
	...
	ZeroMemory( motname, 256 );
	int framenum, mottype;
	*cookieptr = motfile->LoadMotDataFromBuf( m_mhandler, bufptr, bufsize, motname, &framenum, &mottype );
	if( *cookieptr < 0 ){
		DbgOut( "HandlerSet : LoadMotFileFromBuf : LoadMotData error !!!\n" );
		return 1;
	}
	if( framenum > 0 )
		*framemaxptr = framenum - 1;
...
int CHandlerSet::Transform( D3DXVECTOR3 camerapos, LPDIRECT3DDEVICE9 pd3dDevice, D3DXMATRIX matView, 
	int lightflag, int trabone_only )
{
	int ret = 0;
...
int CHandlerSet::Restore( LPDIRECT3DDEVICE9 pd3dDevice, HWND appwnd, int billboardid, CPanda* panda, int pndid )
{
	HRESULT hr;
	D3DCAPS9 caps8;
	hr = pd3dDevice->GetDeviceCaps( &caps8 );
	...
	if( billboardid == -2 ){
		...
		for( seri = 0; seri < m_shandler->s2shd_leng; seri++ ){
			...
			if( (selem->type == SHDPOLYMESH) || (selem->type == SHDPOLYMESH2) ){
				ret = Restore( pd3dDevice, appwnd, seri );
				if( ret ){
		...
		ret = m_shandler->CreateDispObj( pd3dDevice, m_mhandler, 1 );
		if( ret ){
...
int CHandlerSet::SetMotionStep( int motcookie, int step )
{
	...
	if( (motcookie < 0) || (motcookie >= m_mhandler->m_kindnum) ){
		DbgOut( "HandlerSet : SetMotionStep : motid out of range error !!!\n" );
		_ASSERT( 0 );
		return 1;
	}
...
{
	m_gpd.m_matWorld._41 = pos.x;
	m_gpd.m_matWorld._42 = pos.y;
	m_gpd.m_matWorld._43 = pos.z;
...
{
	//m_gpd.m_matWorld = mat;

	//平行移動成分は、コピーしない。
	m_gpd.m_matWorld._11 = mat._11;
	m_gpd.m_matWorld._12 = mat._12;
	m_gpd.m_matWorld._13 = mat._13;

	m_gpd.m_matWorld._21 = mat._21;
	m_gpd.m_matWorld._22 = mat._22;
	m_gpd.m_matWorld._23 = mat._23;

	m_gpd.m_matWorld._31 = mat._31;
	m_gpd.m_matWorld._32 = mat._32;
	m_gpd.m_matWorld._33 = mat._33;
...
int CHandlerSet::SetRotate( D3DXVECTOR3 rot )
{
	...
	CMatrix2 rotmat;
	rotmat = q.MakeRotMat();
	m_gpd.m_matWorld._11 = rotmat.data[0][0];
	m_gpd.m_matWorld._12 = rotmat.data[0][1];
	m_gpd.m_matWorld._13 = rotmat.data[0][2];

	m_gpd.m_matWorld._21 = rotmat.data[1][0];
	m_gpd.m_matWorld._22 = rotmat.data[1][1];
	m_gpd.m_matWorld._23 = rotmat.data[1][2];

	m_gpd.m_matWorld._31 = rotmat.data[2][0];
	m_gpd.m_matWorld._32 = rotmat.data[2][1];
	m_gpd.m_matWorld._33 = rotmat.data[2][2];
...
int CHandlerSet::LoadGroundBMPTS( char* bmpname1, float maxx, float maxz, int divx, int divz, float maxheight )
{
	int ret = 0;
	...
	ret = gbmp->LoadGroundBMPTS( m_thandler, m_shandler, m_mhandler, bmpname1, maxx, maxz, divx, divz, maxheight );
	if( ret ){
		_ASSERT( 0 );
		return 1;
	}
...
int CHandlerSet::ChkConfGround( CHandlerSet* charahs, int mode, float diffmaxy, float mapminy, int* resultptr, float* adxptr, float* adyptr, float* adzptr, float* nxptr, float* nyptr, float* nzptr )
{
	...
	ChkConfGround2( bpos, npos, mode, diffmaxy, mapminy, resultptr, adxptr, adyptr, adzptr, nxptr, nyptr, nzptr );
...
int CHandlerSet::SetNaviLineOnGround( CNaviLine* nl, float mapmaxy, float mapminy )
{
	...
	while( curnp ){
		...
		ret = m_shandler->ChkConfGround( 0, 0, &m_gpd.m_matWorld, befpos, newpos, m_mhandler, 0, 200.0f, -100.0f, &result, &adjustv, &nv );
		if( ret ){
...
int CHandlerSet::GetScreenPos3F( int partno, D3DXVECTOR3* scposptr, D3DXMATRIX matView, int vertno, int calcmode )
{
	...
	ret = m_shandler->GetScreenPos3F( m_billboardflag, m_mhandler, partno, vertno, calcmode, dwClipWidth, dwClipHeight,
		m_gpd.m_matWorld, matView, g_matProj, scposptr );
	if( ret ){
...
//DbgOut( "check!!! pick 0 : getnum %d\r\n", *getnumptr );
//int chkget;
//for( chkget = 0; chkget < *getnumptr; chkget++ ){
//	DbgOut( "check!!! pick 0 : vert %d\r\n", *( vertptr + chkget ) );
//}
...
int CHandlerSet::ChkConfLineAndFacePart( int chkpartno, D3DXVECTOR3 camdir, D3DXMATRIX matView, D3DXVECTOR3 srcpos1, D3DXVECTOR3 srcpos2, int needtrans, 
		int* partptr, int* faceptr, D3DXVECTOR3* dstpos, D3DXVECTOR3* dstn, int* revfaceptr )
{
	int ret;

	if( (chkpartno <= 0) || (chkpartno >= m_shandler->s2shd_leng) ){
		DbgOut( "handlerset : ChkConfLineAndFacePart : chkpartno error !!!\n" );
		_ASSERT( 0 );
		return 1;
	}
	CShdElem* chkselem;
	chkselem = (*m_shandler)( chkpartno );
	if( (chkselem->type != SHDPOLYMESH) && (chkselem->type != SHDPOLYMESH2) ){
		DbgOut( "handlerset : ChkConfLineAndFacePart : chkpartno object type error !!!\n" );
		_ASSERT( 0 );
		return 1;
	}

	ret = m_shandler->ChkConfLineAndFacePart( chkpartno, m_clientWidth, m_clientHeight, m_groundflag, m_mhandler, m_gpd.m_matWorld, matView,
		srcpos1, srcpos2, needtrans, partptr, faceptr, dstpos, dstn, revfaceptr );
	if( ret ){
		DbgOut( "handlerset : ChkConfLineAndFacePart : sh ChkConfLineAndFacePart error !!!\n" );
		_ASSERT( 0 );
		return 1;
	}
...
int CHandlerSet::GetMoaInfo( int maxnum, PVal* pvalstr, APTR aptrstr, int* idptr, int* getnumptr )
{
...
int CHandlerSet::GetJointInfo( int maxnum, PVal* pvalstr, APTR aptrstr, int* seriptr, int* getnumptr )
{
...
int CHandlerSet::GetDispObjInfo( int maxnum, PVal* pvalstr, APTR aptrstr, int* seriptr, int* getnumptr )
{
...
int CHandlerSet::SetUVBatchDouble( int partno, int* vertnoptr, int vertnum, double* uvptr, int setflag, int clampflag )
{
	int ret;

	ret = m_shandler->SetUVBatchDouble( partno, vertnoptr, vertnum, uvptr, setflag, clampflag );
	if( ret ){
		DbgOut( "handlerset : sh SetUVBatchDouble error !!!\n" );
		_ASSERT( 0 );
		return 1;
	}
...
/////////
	...
	ret = m_shandler->GetBonePos( shiftno, 0, motid, frameno, &orgobj, m_mhandler, inimat, 1 );
	if( ret ){
...
int CHandlerSet::CreateEmptyMotion( char* motionname, int frameleng, int* motidptr )
{
	int ret;
	...
	*motidptr = m_mhandler->AddMotion( ZA_1, motionname, motiontype, frameleng, interpolation, motionjump );
	if( *motidptr < 0 ){
	...
	ret = m_mhandler->CalcMPEuler( m_shandler, *motidptr, m_mhandler->GetZaType( *motidptr ) );//!!!!!!!!!!!!!!!
	if( ret ){
...
int CHandlerSet::GetMotionName( PVal* pvalstr, APTR aptrstr, int motid )
{
...
int CHandlerSet::GetMaterial1stTex( PVal* pvalstr, APTR aptrstr, int matid )
{
	CMQOMaterial* curmat;
	curmat = m_shandler->GetMaterialFromNo( m_shandler->m_mathead, matid );
	if( !curmat ){
		DbgOut( "hs : Material1stTex : sh Material1stTex error !!!\n" );
		_ASSERT( 0 );
		return 1;
	}
...
int CHandlerSet::SetMaterial1stTex( LPDIRECT3DDEVICE9 pdev, HWND appwnd, int matid, char* srctexname )
{
	...
	int namelen;
	namelen = (int)strlen( srctexname );
	if( (namelen > 0) && (namelen < 256) ){
		strcpy_s( curmat->tex, 256, srctexname );
	}else{
		_ASSERT( 0 );
		curmat->tex[0] = 0;
	}
...
//		LPDIRECT3DTEXTURE9 dbgtex = 0;
//		dbgtex = g_texbnk->GetTexData( curmat->bump, transparent );
//DbgOut( "handlerset : SetBumpMapToMaterial : %s, %d, %x\n", texname, transparent, dbgtex );
	}else{
		// NULL テクスチャ
		curmat->bump[0] = 0;
//DbgOut( "handlerset : SetBumpMapToMaterial : set NULL \n" );
...
int CHandlerSet::ExtractParts( CHandlerSet* orghs, char** ppname, int extractnum )
{
	int ret;
	int partno[256];
	int donum = 0;
	ZeroMemory( partno, sizeof( int ) * 256 );

	int i;
	for( i = 0; i < extractnum; i++ ){
		int tmpint = -1;
		ret = orghs->m_thandler->GetDispObjNoByName( *(ppname + i), &tmpint, orghs->m_shandler, 0 );
		if( (ret == 0) && (tmpint > 0) ){
			CShdElem* selem;
			selem = (*orghs->m_shandler)( tmpint );
			_ASSERT( selem );
	...
	if( donum <= 0 ){
		DbgOut( "hs : ExtractParts : part not exist error !!!\n" );
		_ASSERT( 0 );
		return 1;
	}
//////////////////////
	m_groundflag = orghs->m_groundflag;

	m_shandler->m_aspect = 1.0f;
	m_shandler->m_im2enableflag = 1;
	m_shandler->m_bonetype = BONETYPE_RDB2;
	m_shandler->m_mikoblendtype = MIKOBLEND_MIX;
	m_mhandler->m_standard = 0;
	//loadmagicno = sh.serialno;//!!!!!!!!!!!!!
	m_shandler->m_sigmagicno = orghs->m_shandler->m_sigmagicno;
	m_shandler->m_scene_ambient = orghs->m_shandler->m_scene_ambient;
	m_shandler->m_shader = orghs->m_shandler->m_shader;
	m_shandler->m_overflow = orghs->m_shandler->m_overflow;

	ZeroMemory( m_shandler->m_dirname, sizeof( char ) * PATH_LENG );
	if( orghs->m_shandler->m_dirname[0] ){
		strcpy_s( m_shandler->m_dirname, PATH_LENG, orghs->m_shandler->m_dirname );
	}

//	ret = m_shandler->InitMQOMaterial();
//	if( ret ){
//		DbgOut( "hs : ExtractParts : sh InitMQOMaterial error !!!\n" );
//		_ASSERT( 0 );
//		return 1;
//	}
	m_shandler->m_materialcnt = orghs->m_shandler->m_materialcnt;

	ret = CopyMQOMaterial( orghs->m_shandler->m_mathead, m_shandler->m_mathead );
	if( ret ){
		DbgOut( "hs : ExtractParts : CopyMQOMaterial error !!!\n" );
		_ASSERT( 0 );
		return 1;
	}
	...
	ret = make_first_part( "パート" );
	_ASSERT( !ret );

	int dono;
	for( dono = 0; dono < donum; dono++ ){
		ret = make_pm2( orghs, partno[dono] );
		if( ret ){
			DbgOut( "hs : ExtractParts : make_pm2 error !!!\n" );
			_ASSERT( 0 );
			return 1;
		}
	}
	...
	ret = make_first_part( "Joint Part" );
	_ASSERT( !ret );
	ret = end();//first_part
	_ASSERT( !ret );
...
///////////
	...
	ret = m_shandler->SetChain( 0 );
	if( ret ){
		DbgOut( "hs : ExtractParts : sh SetChain error !!!\n" );
		_ASSERT( 0 );
		return 1;
	}
	ret = m_shandler->SetColors();
	if( ret ){
		DbgOut( "hs : ExtractParts : sh SetColors error !!!\n" );
		_ASSERT( 0 );
		return 1;
	}
	ret = m_shandler->SetChain( 0 );
	if( ret ){
		DbgOut( "hs : ExtractParts : mh SetChain error !!!\n" );
		_ASSERT( 0 );
		return 1;
	}
...
//	if( ret ){
//		DbgOut( "hs : ExtractParts : sh InfScopeConvTempApplyChild error !!!\n" );
//		_ASSERT( 0 );
//		return 1;
//	}
	...
	ret = m_shandler->SetMikoAnchorApply( m_thandler );
	if( ret ){
		DbgOut( "hs : ExtractParts : sh SetMikoAnchorApply error !!!\n" );
		_ASSERT( 0 );
		return 1;
	}

//	ret = m_shandler->Anchor2InfScope( lpth, lpmh );
//	if( ret ){
//		DbgOut( "hs : ExtractParts : sh Anchor2InfScope error !!!\n" );
//		_ASSERT( 0 );
//		return 1;
//	}
	...
	if( ret ){
		DbgOut( "hs : ExtractParts : sh CreateShdBBoxIfNot error !!!\n" );
		_ASSERT( 0 );
		return 1;
	}

	ret = m_shandler->ConvMorphElem();
	if( ret ){
		DbgOut( "hs : ExtractParts : sh ConvMorphElem error !!!\n" );
		_ASSERT( 0 );
		return 1;
	}
...
int CHandlerSet::CopyMQOMaterial( CMQOMaterial* srcmat, CMQOMaterial* dstmat )
{
	int ret;
	char* exttexlist[MAXEXTTEXNUM];
	char exttexbuf[ MAXEXTTEXNUM ][ 256 ];

	CMQOMaterial* befmat = dstmat;
	CMQOMaterial* cursrcmat = srcmat;
	
	while( cursrcmat ){
		CMQOMaterial* newmat;
		newmat = new CMQOMaterial();
		if( !newmat ){
			DbgOut( "hs : CopyMQOMaterial : newmat alloc error !!!\n" );
			_ASSERT( 0 );
			return 1;
		}
		if( befmat ){
			...
			m_shandler->m_mathead = newmat;
		}
		
		newmat->materialno = cursrcmat->materialno;
		newmat->legacyflag = cursrcmat->legacyflag;
		if( cursrcmat->name[0] != 0 ){
			strcpy_s( newmat->name, 256, cursrcmat->name );
		}
		newmat->col = cursrcmat->col;
		newmat->dif = cursrcmat->dif;
		newmat->amb = cursrcmat->amb;
		newmat->emi = cursrcmat->emi;
		newmat->spc = cursrcmat->spc;
		newmat->power = cursrcmat->power;
		if( cursrcmat->tex[0] != 0 ){
			strcpy_s( newmat->tex, 256, cursrcmat->tex );
		}
		if( cursrcmat->alpha[0] != 0 ){
			strcpy_s( newmat->alpha, 256, cursrcmat->alpha );
		}
		if( cursrcmat->bump[0] != 0 ){
			strcpy_s( newmat->bump, 256, cursrcmat->bump );
		}
		newmat->vcolflag = cursrcmat->vcolflag;
		newmat->shader = cursrcmat->shader;
		newmat->dif4f = cursrcmat->dif4f;
		newmat->amb3f = cursrcmat->amb3f;
		newmat->emi3f = cursrcmat->emi3f;
		newmat->spc3f = cursrcmat->spc3f;

		newmat->transparent = cursrcmat->transparent;
		newmat->texrule = cursrcmat->texrule;
		newmat->blendmode = cursrcmat->blendmode;
		newmat->texanimtype = cursrcmat->texanimtype;

		newmat->uanime = cursrcmat->uanime;
		newmat->vanime = cursrcmat->vanime;

		newmat->exttexnum = cursrcmat->exttexnum;
		newmat->exttexmode = cursrcmat->exttexmode;
		newmat->exttexrep = cursrcmat->exttexrep;
		newmat->exttexstep = cursrcmat->exttexstep;

		newmat->convnamenum = cursrcmat->convnamenum;
		
		newmat->alphatest0 = cursrcmat->alphatest0;
		newmat->alphaval0 = cursrcmat->alphaval0;
		newmat->alphatest1 = cursrcmat->alphatest1;
		newmat->alphaval1 = cursrcmat->alphaval1;

		MoveMemory( newmat->glowmult, cursrcmat->glowmult, sizeof( float ) * 3 );
		newmat->orgalpha = cursrcmat->orgalpha;

		if( newmat->exttexnum > 0 ){
			if( newmat->exttexnum > MAXEXTTEXNUM ){
				DbgOut( "hs : CopyMQOMaterial : exttexnum out of range error !!!\n" );
				_ASSERT( 0 );
				return 1;
			}
			...
			for( texno = 0; texno < newmat->exttexnum; texno++ ){
				char* dstbuf;
				dstbuf = (char*)exttexbuf[texno];
				strcpy_s( dstbuf, 256, *(cursrcmat->exttexname + texno) );
			}

			ret = newmat->SetExtTex( newmat->exttexnum, exttexlist );
			if( ret ){
				DbgOut( "hs : CopyMQOMaterial : mqomat SetExtTex error !!!\n" );
				_ASSERT( 0 );
				return 1;
			}
		}
		...
		if( newmat->convnamenum > 0 ){
			if( newmat->convnamenum > MAXEXTTEXNUM ){
				DbgOut( "hs : CopyMQOMaterial : convnamenum out of range error !!!\n" );
				_ASSERT( 0 );
				return 1;
			}
			...
			for( texno = 0; texno < newmat->convnamenum; texno++ ){
				...
				strcpy_s( dstbuf, 256, *(cursrcmat->ppconvname + texno) );
			}
...
int CHandlerSet::start()
{
	int ret;
	m_depth = 0;
	cur_seri = 0;

	ret = m_thandler->Start( 0 );
	if( ret ){
		_ASSERT( 0 );
		return 1;
	}
...
int CHandlerSet::begin()
{
	int ret;
	ret = m_thandler->Begin();
	if( ret ){
		DbgOut( "hs : begin : th begin error!!!\n" );
		_ASSERT( 0 );
		return 1;
	}

	m_depth = m_thandler->GetCurDepth(); //!!! thandler->Beginするたびに必ず呼ぶ。

	ret = AddShape2Tree();
	if( ret ){
		DbgOut( "hs : begin : AddShape2Tree error !!!\n" );
		_ASSERT( 0 );
		return 1;
	}
	return 0;
...
int CHandlerSet::AddShape2Tree()
{
	...
	ret = m_tempinfo.ResetParams();
	if( ret ){
		DbgOut( "hs : AddShape2Tree : ResetParams error!!!\n" );
		_ASSERT( 0 );
		return 1;
	}

	m_thandler->AddTree( m_tempname, cur_seri );
	...
	ret = m_thandler->SetElemType( cur_seri, m_shdtype );
	if( ret ){
		DbgOut( "hs : AddShape2Tree : thandler->SetElemType error !!!\n" );
		_ASSERT( 0 );
		return 1;
	}
	...
	// shdhandler
	ret = m_shandler->AddShdElem( cur_seri, m_depth );
	if( ret ){
		DbgOut( "hs : AddShape2Tree : shandler->AddShdElem error !!!\n" );
		_ASSERT( 0 );
		return 1;
	}


	ret = m_tempinfo.SetMem( m_shdtype, BASE_TYPE );
	if( ret ){
		DbgOut( "hs : AddShape2Tree : tempinfo.SetMem : BASE_TYPE error !!!\n" );
		_ASSERT( 0 );
		return 1;
	}	

	ret = m_mhandler->AddMotionCtrl( cur_seri, m_depth );
	if( ret ){
		DbgOut( "hs : AddShape2Tree : AddMotionCtrl error !!!\n" );
		_ASSERT( 0 );
		return 1;
	}
	...
	//hasmotionのセット
		//skip jointの際に使う。	
	if( m_shdtype == SHDBALLJOINT ){
		ret = m_mhandler->SetHasMotion( cur_seri, 1 );
		if( ret ){
			DbgOut( "hs : AddShape2Tree : SetHasMotion error !!!\n" );
			_ASSERT( 0 );
			return 1;
		}
	}else{
		ret = m_mhandler->SetHasMotion( cur_seri, 0 );
		if( ret ){
			DbgOut( "hs : AddShape2Tree : SetHasMotion error !!!\n" );
			_ASSERT( 0 );
			return 1;
...
int CHandlerSet::make_first_part( char* srcname )
{
	int ret;

	m_shdtype = SHDPART;
	strcpy_s( m_tempname, 256, srcname );

	ret = begin();
	if( ret ){
		DbgOut( "hs : make_first_part : begin error !!!\n" );
		_ASSERT( 0 );
		return 1;
	}

	ret = m_shandler->Init3DObj( cur_seri, &m_tempinfo );
	if( ret ){
		DbgOut( "hs : make_first_part : Init3DObj error !!!\n" );
		_ASSERT( 0 );
		return 1;
	}
...
int CHandlerSet::make_pm2( CHandlerSet* orghs, int partno )
{
	int ret;

	CTreeElem2* orgte = (*orghs->m_thandler)( partno );
	_ASSERT( orgte );
	CShdElem* orgse = (*orghs->m_shandler)( partno );
	_ASSERT( orgse );

	if( orgse->type != SHDPOLYMESH2 ){
		_ASSERT( 0 );
		return 1;
	}

	strcpy_s( m_tempname, 256, orgte->name );
	m_shdtype = SHDPOLYMESH2;
	ret = begin();
	if( ret ){
		DbgOut( "hs : make_pm2 : begin error !!!\n" );
		_ASSERT( 0 );
		return 1;
	}
	
	CShdElem* dstse = (*m_shandler)( cur_seri );
	_ASSERT( dstse );

	dstse->clockwise = orgse->clockwise;
...
//		}
//	}
	ret = (*m_shandler)( cur_seri )->SetTexRepetition( orgse->texrepx, orgse->texrepy );
	_ASSERT( !ret );
	ret = (*m_shandler)( cur_seri )->SetTexTransparent( orgse->transparent );
	_ASSERT( !ret );
	ret = (*m_shandler)( cur_seri )->SetUVAnime( orgse->uanime, orgse->vanime );
	_ASSERT( !ret );
	ret = (*m_shandler)( cur_seri )->SetAlpha( orgse->alpha );
	_ASSERT( !ret );
	ret = (*m_shandler)( cur_seri )->SetTexRule( orgse->texrule );
	_ASSERT( !ret );
	ret = (*m_shandler)( cur_seri )->SetNotUse( orgse->notuse );
	_ASSERT( !ret );
	dstse->noskinning = orgse->noskinning;
	dstse->dispswitchno = orgse->dispswitchno;
...
//	}
	dstse->invisibleflag = orgse->invisibleflag;
//	if( sheader.flags & FLAGS_ANCHORFLAG ){
//		(*lpsh)( curseri )->m_anchorflag = 1;
//	}else{
//		(*lpsh)( curseri )->m_anchorflag = 0;
//	}
	dstse->m_mikodef = orgse->m_mikodef;
	dstse->m_userint1 = orgse->m_userint1;
//	if( sheader.iklevel > 0 ){
//		(*lpsh)( curseri )->m_iklevel = sheader.iklevel;
//	}else{
//		(*lpsh)( curseri )->m_iklevel = 5;
//	}
	dstse->facet = orgse->facet;

//////////
	CMeshInfo* orgmi = orgse->GetMeshInfo();
	_ASSERT( orgmi );

	ret = m_tempinfo.SetMem( &(orgmi->type), BASE_TYPE );
	_ASSERT( !ret );
	ret = m_tempinfo.SetMem( orgmi->m, MESHI_M );
	_ASSERT( !ret );
	ret = m_tempinfo.SetMem( orgmi->n, MESHI_N );
	_ASSERT( !ret );
	ret = m_tempinfo.SetMem( 0, MESHI_TOTAL );
	_ASSERT( !ret );
	ret = m_tempinfo.SetMem( 1, MESHI_MCLOSED );
	_ASSERT( !ret );
	ret = m_tempinfo.SetMem( 0, MESHI_NCLOSED );
	_ASSERT( !ret );

	ret = m_shandler->Init3DObj( cur_seri, &m_tempinfo );
	if( ret ){
		DbgOut( "hs : make_pm2 : Init3DObj error !!!\n" );
		_ASSERT( !ret );
		return 1;
	}
	...
	CPolyMesh2* orgpm2;
	orgpm2 = orgse->polymesh2;
	_ASSERT( orgpm2 );

	pm2->groundflag = orgpm2->groundflag;

//////////////
	int facenum;
	facenum = m_tempinfo.n;
	pm2->adjustuvflag = orgpm2->adjustuvflag;
	MoveMemory( pm2->pointbuf, orgpm2->pointbuf, sizeof( VEC3F ) * facenum * 3 );
	MoveMemory( pm2->samepointbuf, orgpm2->samepointbuf, sizeof( int ) * facenum * 3 );
	MoveMemory( pm2->oldpno2optpno, orgpm2->oldpno2optpno, sizeof( int ) * facenum * 3 );
	MoveMemory( pm2->smoothbuf, orgpm2->smoothbuf, sizeof( int ) * facenum * 3 );
	MoveMemory( pm2->uvbuf, orgpm2->uvbuf, sizeof( COORDINATE ) * facenum * 3 );
	
	MoveMemory( pm2->diffusebuf, orgpm2->diffusebuf, sizeof( ARGBF ) * facenum );
	MoveMemory( pm2->ambientbuf, orgpm2->ambientbuf, sizeof( ARGBF ) * facenum );
	MoveMemory( pm2->specularbuf, orgpm2->specularbuf, sizeof( float ) * facenum );
	MoveMemory( pm2->orgnormal, orgpm2->orgnormal, sizeof( VEC3F ) * facenum * 3 );
	pm2->m_loadorgnormalflag = 1;//!!!!!!!!!!!!!!!!!!!!!!

DbgOut( "check!!!: hs : make_pm2 0 %s\n", orgte->name );

	ret = pm2->CreateOptDataByOld2Opt( pm2->adjustuvflag, orgse->facet, 0 );
	if( ret ){
		DbgOut( "hs : make_pm2 : CreateOptData error !!!\n" );
		_ASSERT( 0 );
		return 1;
	}

//DbgOut( "check!!!: hs : make_pm2 1\n" );

	int optpleng = pm2->optpleng;
	if( optpleng != orgpm2->optpleng ){
		DbgOut( "hs : make_pm2 : optpleng error !!!\n" );
		_ASSERT( 0 );
		return 1;
	}

//DbgOut( "check!!!: hs : make_pm2 2\n" );

	MoveMemory( pm2->opttlv, orgpm2->opttlv, sizeof( D3DTLVERTEX ) * optpleng );
	MoveMemory( pm2->optambient, orgpm2->optambient, sizeof( ARGBF ) * optpleng );
	
	MoveMemory( pm2->diffbuf, orgpm2->diffbuf, sizeof( float ) * facenum );
	MoveMemory( pm2->powerbuf, orgpm2->powerbuf, sizeof( float ) * facenum );
	MoveMemory( pm2->emissivebuf, orgpm2->emissivebuf, sizeof( float ) * facenum );

	MoveMemory( pm2->optpowerbuf, orgpm2->optpowerbuf, sizeof( float ) * optpleng );
	MoveMemory( pm2->optemissivebuf, orgpm2->optemissivebuf, sizeof( ARGBF ) * optpleng );

	MoveMemory( pm2->orgnobuf, orgpm2->orgnobuf, sizeof( int ) * facenum * 3 );

//DbgOut( "check!!!: hs : make_pm2 3\n" );

	ret = pm2->CreateInfElemIfNot( optpleng );
	if( ret ){
		DbgOut( "hs : make_pm2 : CreateInfElemIfNot error !!!\n" );
		_ASSERT( 0 );
		return 1;
	}

//DbgOut( "check!!!: hs : make_pm2 4\n" );

	//set infelem

	int vcno;
	for( vcno = 0; vcno < orgpm2->vcolnum; vcno++ ){
		VCOLDATA* curvc;
		curvc = *( orgpm2->vcoldata + vcno );

		ret = pm2->SetVCol( curvc->vertno, curvc->vcol );
		if( ret ){
			DbgOut( "hs : make_pm2 : pm2 SetVCol error !!!\n" );
			_ASSERT( 0 );
			return 1;
		}
	}
	...
	pm2->m_materialnum = orgpm2->m_materialnum;
	if( orgpm2->m_toon1 ){
DbgOut( "check!!!: hs : make_pm2 5\n" );
		pm2->m_toon1 = new CToon1Params[ pm2->m_materialnum ];
		if( !pm2->m_toon1 ){
			DbgOut( "hs : make_pm2 : m_toon1 alloc error !!!\n" );
			_ASSERT( 0 );
			return 1;
		}
		int matno;
		for( matno = 0; matno < pm2->m_materialnum; matno++ ){
			...
			CToon1Params* orgtoon1;
			orgtoon1 = orgpm2->m_toon1 + matno;

			strcpy_s( curtoon1->name, 32, orgtoon1->name );
			curtoon1->diffuse = orgtoon1->diffuse;
			curtoon1->ambient = orgtoon1->ambient;
			curtoon1->specular = orgtoon1->specular;
			curtoon1->darkh = orgtoon1->darkh;
			curtoon1->brighth = orgtoon1->brighth;
			curtoon1->ambientv = orgtoon1->ambientv;
			curtoon1->diffusev = orgtoon1->diffusev;
			curtoon1->specularv = orgtoon1->specularv;
			curtoon1->darknl = orgtoon1->darknl;
			curtoon1->brightnl = orgtoon1->brightnl;
			curtoon1->tex = 0;
			curtoon1->edgecol0 = orgtoon1->edgecol0;
			curtoon1->edgevalid0 = orgtoon1->edgevalid0;
			curtoon1->edgeinv0 = orgtoon1->edgeinv0;

			curtoon1->toon0dnl = orgtoon1->toon0dnl;
			curtoon1->toon0bnl = orgtoon1->toon0bnl;
		}

		if( orgpm2->m_attrib ){
DbgOut( "check!!!: hs : make_pm2 6\n" );

			pm2->m_attrib = new DWORD[ facenum ];
			if( !pm2->m_attrib ){
				DbgOut( "hs : make_pm2 : attrib alloc error !!!\n" );
				_ASSERT( 0 );
				return 1;
			}
	//DbgOut( "check!!!: hs : make_pm2 61\n" );
			_ASSERT( orgpm2->m_attrib );

			MoveMemory( pm2->m_attrib, orgpm2->m_attrib, sizeof( DWORD ) * facenum );
			if( !pm2->m_attrib0 ){
				DbgOut( "hs : make_pm2 : attrib0 NULL error !!!\n" );
				_ASSERT( 0 );
				return 1;
			}
		}
...
//DbgOut( "check!!!: hs : make_pm2 62\n" );

	MoveMemory( pm2->m_attrib0, orgpm2->m_attrib0, sizeof( int ) * facenum );

//DbgOut( "check!!!: hs : make_pm2 63\n" );
...
//DbgOut( "check!!!: hs : make_pm2 7\n" );

///////////////
	ret = end();
	if( ret ){
		DbgOut( "hs : make_pm2 : end error !!!\n" );
		_ASSERT( 0 );
		return 1;
	}

//DbgOut( "check!!!: hs : make_pm2 8\n" );
...
int CHandlerSet::GetMOATrunkNotComID( int motid, int arrayleng, int* dstid, int* getnum )
{
	if( !m_mch ){
		DbgOut( "hs : GetMOATrunkNotComID : mch NULL error !!!\n" );
		_ASSERT( 0 );
		return 1;
	}

	int ret;
	ret = m_mch->GetTrunkNotComID( motid, arrayleng, dstid, getnum );
	if( ret ){
		DbgOut( "hs : GetMOATrunkNotComID : mch GetTruncNotComID error !!!\n" );
		_ASSERT( 0 );
		return 1;
	}
...
int CHandlerSet::GetZa3Euler( int motid, int frameno, int boneno, D3DXVECTOR3 befeul, D3DXVECTOR3* dsteul )
{
	if( !m_thandler || !m_shandler || !m_mhandler )
		return 0;
/////////////
	if( m_mhandler->m_kindnum <= 0 ){
		DbgOut( "handlerset : GetZa3Euler : motion not loaded error !!!\n" );
		_ASSERT( 0 );
		return 1;
	}

	if( (motid < 0) || (motid >= m_mhandler->m_kindnum) ){
		DbgOut( "handlerset : GetZa3Euler : motid error !!!\n" );
		_ASSERT( 0 );
		return 1;
	}

	if( (boneno <= 0) || (boneno >= m_mhandler->s2mot_leng) ){
		DbgOut( "handlerset : GetZa3Euler : boneno out of range error !!!\n" );
		_ASSERT( 0 );
		return 1;
	}

	if( (frameno < 0) || (frameno >= *(m_mhandler->m_motnum + motid)) ){
		DbgOut( "mothandler : GetZa3Euler : frameno out of range error !!!\n" );
		_ASSERT( 0 );
		return 1;
	}
...
/////////////
	...
	CMotionCtrl* mcptr = (*m_mhandler)( boneno );
	_ASSERT( mcptr );
	CShdElem* selem = (*m_shandler)( boneno );
	_ASSERT( selem );

	if( selem->IsJoint() == 0 ){
		DbgOut( "hs : GetZa3Euler : this parts is not bone error !!!\n" );
		return 0;
	}
	int hasmp = 0;
	CMotionPoint2 mp2;
	ret = mcptr->CalcMotionPointOnFrame( selem, &mp2, motid, frameno, &hasmp, m_shandler, m_mhandler );
	if( ret ){
		DbgOut( "hs : GetZa3Euler : mc CalcMotionPointOnFrame error !!!\n" );
		_ASSERT( 0 );
		return 1;
	}

	CQuaternion axisq;
	ret = m_shandler->GetInitialBoneQ( boneno, &axisq );
	if( ret ){
		DbgOut( "hs : GetZa3Euler : sh GetInitialBoneQ error !!!\n" );
		_ASSERT( 0 );
		return 1;
	}

	D3DXVECTOR3 tmpeul;
	ret = qToEulerAxis( axisq, &(mp2.m_q), &tmpeul );
	if( ret ){
		DbgOut( "hs : GetZa3Euler : qToEulerAxis error !!!\n" );
		_ASSERT( 0 );
		return 1;
	}

	ret = modifyEuler( &tmpeul, &befeul );
	if( ret ){
		DbgOut( "hs : GetZa3Euler : modifyEuler error !!!\n" );
		_ASSERT( 0 );
		return 1;
	}
...
int CHandlerSet::SetZa3Euler( int motid, int frameno, int boneno, D3DXVECTOR3 seteul )
{
	if( !m_thandler || !m_shandler || !m_mhandler )
		return 0;
/////////////
	if( m_mhandler->m_kindnum <= 0 ){
		DbgOut( "handlerset : SetZa3Euler : motion not loaded error !!!\n" );
		_ASSERT( 0 );
		return 1;
	}

	if( (motid < 0) || (motid >= m_mhandler->m_kindnum) ){
		DbgOut( "handlerset : SetZa3Euler : motid error !!!\n" );
		_ASSERT( 0 );
		return 1;
	}

	if( (boneno <= 0) || (boneno >= m_mhandler->s2mot_leng) ){
		DbgOut( "handlerset : SetZa3Euler : boneno out of range error !!!\n" );
		_ASSERT( 0 );
		return 1;
	}

	if( (frameno < 0) || (frameno >= *(m_mhandler->m_motnum + motid)) ){
		DbgOut( "mothandler : SetZa3Euler : frameno out of range error !!!\n" );
		_ASSERT( 0 );
		return 1;
	}
...
/////////////
	...
	if( selem->IsJoint() == 0 ){
		DbgOut( "hs : SetZa3Euler : this parts is not bone error !!!\n" );
		return 0;
	}
	...
	ret = m_shandler->GetInitialBoneQ( boneno, &axisq );
	if( ret ){
		DbgOut( "hs : SetZa3Euler : sh SetInitialBoneQ error !!!\n" );
		_ASSERT( 0 );
		return 1;
	}
	axisq.inv( &invaxisq );
	...
	qx.SetAxisAndRot( axisX, seteul.x * (float)DEG2PAI );
	qy.SetAxisAndRot( axisY, seteul.y * (float)DEG2PAI );
	qz.SetAxisAndRot( axisZ, seteul.z * (float)DEG2PAI );
	...
	if( curmp ){
		...
		if( !newmp ){
			_ASSERT( 0 );
			DbgOut( "handlerset : SetZa3Euler : AddMotionPoint error" );
			return 1;
		}
	}
...
int CHandlerSet::GetMaterialExtTexture( PVal* pvalstr, APTR aptrstr, int matno, int extno )
{
	CMQOMaterial* curmat;
	curmat = m_shandler->GetMaterialFromNo( m_shandler->m_mathead, matno );
	if( !curmat ){
