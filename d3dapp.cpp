...
static LRESULT CALLBACK tmpMsgProc( HWND hWnd, UINT uMsg, WPARAM wParam,
                                  LPARAM lParam )
{
	return 0;
}
...
//m_d3dpp.PresentationInterval = D3DPRESENT_INTERVAL_DEFAULT;
	...
	if( SUCCEEDED(hr) )
    {
		...
		g_numrts = m_d3dCaps.NumSimultaneousRTs;
//g_numrts = 1;
...
DbgOut( "d3dapp : Resize3DEnvironment 0 \r\n" );
...
DbgOut( "d3dapp : Resize3DEnvironment 1 \r\n" );
/// 作り直す。
	if( m_pZSurf ){
		m_pZSurf->Release();
		m_pZSurf = 0;
...
DbgOut( "d3dapp : Resize3DEnvironment 2 \r\n" );
    // Reset the device
...
DbgOut( "d3dapp : Resize3DEnvironment 3 \r\n" );
	hr = m_pd3dDevice->GetDepthStencilSurface( &m_pZSurf );
	if( hr != D3D_OK ){
		DbgOut( "d3dapp : Initialize3DEnvironment : dev GetDepthStencilSurface error !!!\n" );
		_ASSERT( 0 );
...
DbgOut( "d3dapp : Resize3DEnvironment 4 \r\n" );

    // Initialize the app's device-dependent objects
    hr = RestoreDeviceObjects( resetflag );
	if( FAILED(hr) ){
		_ASSERT( 0 );
        return hr;
	}
...
DbgOut( "d3dapp : Resize3DEnvironment 5 \r\n" );
...
int CD3DApplication::TestCooperative()
{
    ...
    if( FAILED( hr = m_pd3dDevice->TestCooperativeLevel() ) )
    {
        // If the device was lost, do not render until we get it back
		if( D3DERR_DEVICELOST == hr ){
DbgOut( "d3dapp : TestCooperative : DEVICELOST !!!\n" );
			return 2;//!!!!!!!!!!!!!!!
		}
...
DbgOut( "d3dapp : TestCooperative : ERR_DEVICENOTRESET !!!\n" );
...
DbgOut( "d3dapp : TestCooperative Resize3DEnvironment !!!\n" );
				if( FAILED( hr = Resize3DEnvironment() ) ){
					_ASSERT( 0 );
					return 1;//!!!!!!!!!!
				}
...
HRESULT CD3DApplication::RestoreDeviceObjects( int resetflag )
{
	if( m_pFont )
