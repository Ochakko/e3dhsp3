...
***/
	...
	int createpool = D3DPOOL_MANAGED;

	if( spritename[0] != 0 ){
		
		char rtpat[30] = "RenderTarget_";
		int cmprt;
		cmprt = strncmp( rtpat, spritename, 13 );
		if( cmprt == 0 ){
			createpool = D3DPOOL_DEFAULT;
		}else{
			createpool = D3DPOOL_MANAGED;
		}

		addflag = g_texbnk->AddName( 0, spritename, texbanktransparent, createpool );
		//ret = g_texbnk->AddName( spritename, texbanktransparent, D3DPOOL_DEFAULT );
		if( addflag < 0 ){
			//エラー
			DbgOut( "mysprite : RestoreTexture : texbank AddName error !!! flag %d, trans %d, %s\r\n",
				addflag, texbanktransparent, spritename );
			_ASSERT( 0 );
			return 1;
		}
...
//		if( addflag != 0 ){//0のときはすでにテクスチャがある
			...
			if( (findno < 0) || (findno >= g_texbnk->namenum) ){
				DbgOut( "mysprite : RestoreTexture : GetTexNoByName error !!! %s\n", spritename );
				_ASSERT( 0 );
				return 1;
			}
			...
			if( ret && (ret != -2) ){
				...
				if( ret2 ){
					DbgOut( "mysprite : RestoreTexture : SetTexData NULL error !!!\n" );
					_ASSERT( 0 );
					return 1;
				}
			}
//		}
	...
	if( !pd3dtex ){
		DbgOut( "mysprite : RestoreTexture : aft create : pd3dtex NULL warning !!! %s\r\n", spritename );
	}
...
////////////
	...
	if( pd3dtex ){
		...
		if( hr != D3D_OK ){
			DbgOut( "mysprite : RestoreTexture : GetLevelDesc 1 error !!!\n" );
			_ASSERT( 0 );
			return 1;
		}
	...
	//	if( addflag != 0 ){
			if( transparentflag && pd3dtex && (createpool == D3DPOOL_MANAGED) ){
				DbgOut( "mysprite : RestoreTexture : SetTextureAlpha start\r\n" );
				ret = SetTextureAlpha();
				if( ret ){
					DbgOut( "mysprite : RestoreTexture : SetTextureAlpha error !!! %s\r\n", spritename );
					_ASSERT( 0 );
					return 1;
				}
				DbgOut( "mysprite : RestoreTexture : SetTextureAlpha end\r\n" );
			}
	//	}
