...
public:
	...
	int LoadGroundBMPTS( char* bmpname1, float maxx, float maxz, int divx, int divz, float maxheight );
	...
	int Transform( D3DXVECTOR3 camerapos, LPDIRECT3DDEVICE9 pd3dDevice, 
		D3DXMATRIX matView, int lightflag, int trabone_only );
	...
	int Restore( LPDIRECT3DDEVICE9 pd3dDevice, HWND appwnd, int billboardid, CPanda* panda = 0, int pndid = 0 );
...
//	int LeaveFromChain();
	...
	int GetScreenPos3F( int partno, D3DXVECTOR3* scposptr, D3DXMATRIX matView, int vertno, int calcmode );
	...
	int ChkConfLineAndFacePart( int chkpartno, D3DXVECTOR3 camdir, D3DXMATRIX matView, D3DXVECTOR3 srcpos1, D3DXVECTOR3 srcpos2, int needtrans, 
		int* partptr, int* faceptr, D3DXVECTOR3* dstpos, D3DXVECTOR3* dstn, int* revfaceptr );
	...
	int SetUVBatchDouble( int partno, int* vertnoptr, int vertnum, double* uvptr, int setflag, int clampflag );
	...
	int CreateEmptyMotion( char* motname, int frameleng, int* motidptr );
	...
	int GetMaterial1stTex( PVal* pvalstr, APTR aptrstr, int matid );
	int SetMaterial1stTex( LPDIRECT3DDEVICE9 pdev, HWND appwnd, int matid, char* srctexname );
	...
	int ExtractParts( CHandlerSet* orghs, char** ppname, int extractnum );
	...
	int GetMOATrunkNotComID( int motid, int arrayleng, int* dstid, int* getnum );
	int SetMOATrunkNotComID( int motid, int arrayleng, int* srcid );

	int GetZa3Euler( int motid, int frameno, int boneno, D3DXVECTOR3 befeul, D3DXVECTOR3* dsteul );
	int SetZa3Euler( int motid, int frameno, int boneno, D3DXVECTOR3 seteul );
	...
	int GetMaterialExtTexture( PVal* pvalstr, APTR aptrstr, int matno, int extno );
...
private:
	...
	int CopyMQOMaterial( CMQOMaterial* srcmat, CMQOMaterial* dstmat );

	int start();
	int begin();
	int end();
	int AddShape2Tree();
	int make_first_part( char* srcname );
	int make_pm2( CHandlerSet* orghs, int partno );
